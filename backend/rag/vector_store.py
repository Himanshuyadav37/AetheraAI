...
class PineconeVectorStore(BaseVectorStore):
    def __init__(self):
        self._index = None
        self._pc = None
        self._fallback_chroma = None
        self._init_client()

    def _get_fallback(self) -> ChromaVectorStore:
        if self._fallback_chroma is None:
            self._fallback_chroma = ChromaVectorStore()
    ...
    def _init_client(self):
        ...
        self._pc = Pinecone(api_key=api_key.strip())
        ...
        try:
            ...
            if index_name not in existing_indexes:
                ...
                dim = len(sample_emb) if sample_emb else 3072
    ...
    def add(self, collection_name: str, ids: List[str], documents: List[str], embeddings: List[List[float]], metadatas: List[dict]):
        try:
            ...
            logger.warning(f"Pinecone add failed ({e}). Falling back to local Chroma store.")
            self._get_fallback().add(collection_name, ids, documents, embeddings, metadatas)
    ...
    def query(self, collection_name: str, query_embeddings: List[List[float]], n_results: int, where: dict = None) -> dict:
        try:
            ...
            logger.warning(f"Pinecone query failed ({e}). Falling back to local Chroma store.")
            return self._get_fallback().query(collection_name, query_embeddings, n_results, where)
    ...
    def get(self, collection_name: str, ids: List[str] = None, where: dict = None, limit: int = None, include: List[str] = None) -> dict:
        try:
            ...
            if ids:
                ...
                dim = len(sample) if sample else 3072
            ...
            logger.warning(f"Pinecone get failed ({e}). Falling back to local Chroma store.")
            return self._get_fallback().get(collection_name, ids, where, limit, include)
    ...
    def delete(self, collection_name: str, ids: List[str] = None, where: dict = None):
        try:
            ...
            logger.warning(f"Pinecone delete failed ({e}). Falling back to Chroma.")
            try:
                self._get_fallback().delete(collection_name, ids, where)
            except Exception:
                pass
    ...
    def delete_collection(self, collection_name: str) -> bool:
        try:
            ...
            logger.warning(f"Pinecone delete namespace '{collection_name}' failed: {e}. Trying Chroma.")
            try:
                return self._get_fallback().delete_collection(collection_name)
            except Exception:
                return False
    ...
    def count(self, collection_name: str) -> int:
        try:
            ...
            try:
                return self._get_fallback().count(collection_name)
            except Exception:
                pass
        return 0
