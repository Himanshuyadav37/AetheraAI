...
def push_project_to_github(project_id: str, repo_name: str, description: str, private: bool = True, custom_token: str | None = None) -> str:
    ...
    try:
        run_git(["config", "user.name", "Aethera Agent"])
        run_git(["config", "user.email", "agent@aethera.ai"])
    except Exception:
        pass
    ...
    run_git(["add", "."])
    try:
        run_git(["commit", "-m", "Initial commit from Aethera AI Workspace"])
    except Exception as e:
